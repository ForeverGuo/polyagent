...
from agents.app_tester import app_tester_node
...
def router_node(state: AgentState):
    ...
    prompt = (
        "判断用户的意图，只返回一个单词：\n"
        "- 如果用户想要测试 Android/iOS App、原生应用、打开某个包名进行测试，返回：app\n"
...
def route_after_planner(state: AgentState):
    """ui_planner 出口：按 intent 分流到 ui_tester 或 app_tester。"""
    return "app_tester" if state.get("intent") == "app" else "ui_tester"


def route_after_advance(state: AgentState):
    """ui_advance 出口：按 intent 回到对应 tester。"""
    return "app_tester" if state.get("intent") == "app" else "ui_tester"


def route_after_app_tools(state: AgentState):
    """
    app_tools 出口：与 route_after_ui_tools 同构。
      - app_close 且是最后子任务 → END
      - app_close 但还有子任务   → ui_advance（自动重启 session）
      - write_file 且是最后子任务 → END
      - 其他                    → app_tester
    """
...
def route_after_app_tester(state: AgentState):
    """app_tester 出口：逻辑与 route_after_ui_tester 一致。"""
    last_msg = state["messages"][-1]
    if last_msg.tool_calls:
        return "app_tools"
    ui_plan = state.get("ui_plan", [])
...
workflow.add_node("app_tester",    app_tester_node)    # App 测试执行，控制 Appium 逐步操作
...
workflow.add_node("app_tools",     ToolNode([*ALL_APP_TOOLS, write_file]))              # Appium 操作 + 保存报告
...
# ui / app 链路：planner 与 advance 按 intent 分流到 ui_tester 或 app_tester
workflow.add_conditional_edges(
    "ui_planner", route_after_planner,
    {"ui_tester": "ui_tester", "app_tester": "app_tester"},
)
workflow.add_conditional_edges(
    "ui_advance", route_after_advance,
    {"ui_tester": "ui_tester", "app_tester": "app_tester"},
)
...
workflow.add_conditional_edges(
    "router", route_by_intent,
    {
        ...
        "app":     "ui_planner",   # App 测试 → 复用 ui_planner 拆任务
...
# app 链路出口
workflow.add_conditional_edges("app_tools",    route_after_app_tools)  # → app_tester | ui_advance | END
workflow.add_conditional_edges("app_tester",   route_after_app_tester) # → app_tools | ui_advance | END
